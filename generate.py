...
def create_html_pdf_report2(lab_dict):
    ...
    with open(lab_dict['lab_path'] + '-returned.html', 'w') as stud_report:
        ...
        stud_report.write('I found that your lab type is \'{}\' and it\'s number is {} <br/>'.format(lab_dict['type'], lab_dict['num']))
...
def generate_answers3(lid, att, year, semester, db_name='./grades.sqlite3'):
    ...
    try:
        shutil.rmtree(os.path.join(dir_name, 'Answers'), ignore_errors=True)
        os.remove(dir_name + "grades.csv")
        os.remove(dir_name + "grades_for_" + correctd_lab_type + "lab_num.csv")
    except Exception as e:
        print('Exception during dir preparetion : ', e)
    ...
    with open(os.path.join(dir_name, 'grades.csv'), 'w') as grades_file:
        for stud_id in all_ids:
            if stud_id not in not_subm_ids:
                grades_file.write("%s, %.1f \n".format(stud_id, grade_dict[stud_id]))
            else:
...
def time_to_str_with_tz(in_time):
    return datetime.utcfromtimestamp(in_time).replace(tzinfo=tz.tzutc()).astimezone(tz.tzlocal()).strftime('%Y-%m-%d %H:%M:%S')
